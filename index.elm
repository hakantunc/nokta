...
row i =
    initialize size (\j -> ( i, j, Blank ))


board =
    initialize size (\i -> row i)


model : Model
model =
    { board = board, turn = WhitePlayer }
...
update (Update x y) model =
    let
        clr =
            if model.turn == WhitePlayer then
                White
            else
                Black
...
view model =
    div []
        [ div [] [ text ("Whose turn?¿: " ++ (toString model.turn)) ]
        ...
        clr =
            case stt of
                ...
                Black ->
                    "black"
    in
        circle [ onClick (Update x y), fill clr, stroke "black", cx xx, cy yy, r rr ] []
